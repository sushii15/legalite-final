...
// Read all JSON files
const parsed = JSON.parse(readFileSync('./aibe4_questions_parsed.json', 'utf-8'))
const missing = JSON.parse(readFileSync('./aibe4_missing.json', 'utf-8'))
...
// Write final file
writeFileSync('./aibe4_all_100_questions_final.json', JSON.stringify(unique, null, 2))

console.log(`✅ Merged all AIBE 4 sources`)
...
if (missing_qs.length > 0) {
  console.log(`⚠️  Missing (${missing_qs.length}): ${missing_qs.join(', ')}`)
} else {
  console.log(`✅ ALL 100 AIBE 4 QUESTIONS COMPLETE!`)
}
