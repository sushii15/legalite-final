...
const aibeText = readFileSync('./aibe5_extracted.txt', 'utf-8').replace(/\r/g, '')
...
writeFileSync('./aibe5_questions_parsed.json', JSON.stringify(questions, null, 2))

console.log(`✅ Parsed AIBE 5: ${questions.length}/100 questions`)
...
if (missing.length > 0) {
  console.log(`⚠️  Missing (${missing.length}): ${missing.slice(0, 20).join(', ')}${missing.length > 20 ? '...' : ''}`)
}
