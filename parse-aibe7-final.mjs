...
while (i < lines.length) {
  ...
  if (qMatch) {
    ...
    // Read next lines to find options and correct answer
    i++
    let optionBuffer = ''
    ...
    while (i < lines.length && !foundCorrectAnswer) {
      ...
      } else if (!nextLine.startsWith('Solution') && !nextLine.startsWith('Quick Tip')) {
    ...
    while (i < lines.length) {
      ...
      if (nextLine.startsWith('Solution') || nextLine.startsWith('Step 1')) {
        i++
        let expText = ''
        while (i < lines.length) {
          const sLine = lines[i].trim()

          // Stop at Quick Tip or next question
