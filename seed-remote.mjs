...
// Papers for AIBE 20-5
const papersData = [
  { aibe_id: 20, title: 'AIBE 20', question_count: 100, is_free: true, year: 2024 },
  { aibe_id: 19, title: 'AIBE 19', question_count: 100, is_free: false, year: 2023 },
  { aibe_id: 18, title: 'AIBE 18', question_count: 100, is_free: false, year: 2023 },
  { aibe_id: 17, title: 'AIBE 17', question_count: 100, is_free: false, year: 2022 },
  { aibe_id: 16, title: 'AIBE 16', question_count: 100, is_free: false, year: 2022 },
  { aibe_id: 15, title: 'AIBE 15', question_count: 100, is_free: false, year: 2021 },
  { aibe_id: 14, title: 'AIBE 14', question_count: 100, is_free: false, year: 2021 },
  { aibe_id: 13, title: 'AIBE 13', question_count: 100, is_free: false, year: 2020 },
  { aibe_id: 12, title: 'AIBE 12', question_count: 100, is_free: false, year: 2020 },
  { aibe_id: 11, title: 'AIBE 11', question_count: 100, is_free: false, year: 2019 },
  { aibe_id: 10, title: 'AIBE 10', question_count: 100, is_free: false, year: 2019 },
  { aibe_id: 9, title: 'AIBE 9', question_count: 100, is_free: false, year: 2018 },
  { aibe_id: 8, title: 'AIBE 8', question_count: 100, is_free: false, year: 2018 },
  { aibe_id: 7, title: 'AIBE 7', question_count: 100, is_free: false, year: 2017 },
  { aibe_id: 6, title: 'AIBE 6', question_count: 100, is_free: false, year: 2017 },
  { aibe_id: 5, title: 'AIBE 5', question_count: 100, is_free: false, year: 2016 },
];

// Sample questions for AIBE 20
const questionsData = [
...
async function seedDatabase() {
  try {
    ...
    // Insert papers
    console.log('1️⃣  Inserting papers...');
    const papersRes = await fetch(`${supabaseUrl}/rest/v1/papers`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify(papersData)
    });
    if (papersRes.status !== 201) {
      const err = await papersRes.json();
      console.log('❌ Papers insert failed:', err.message);
      return;
    }
    console.log(`✅ Inserted ${papersData.length} papers (AIBE 20-5)\n`);

    // Insert questions
    console.log('2️⃣  Inserting sample questions for AIBE 20...');
    const questionsRes = await fetch(`${supabaseUrl}/rest/v1/questions`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${serviceRoleKey}`,
        'apikey': serviceRoleKey,
    ...
    if (questionsRes.status !== 201) {
      const err = await questionsRes.json();
      console.log('❌ Questions insert failed:', err.message);
      return;
    }
    console.log(`✅ Inserted ${questionsData.length} sample questions\n`);

    // Verify with anon key
    console.log('3️⃣  Verifying with anon key...');

    const papersCheckRes = await fetch(`${supabaseUrl}/rest/v1/papers?limit=5`, {
      headers: {
    ...
    console.log(`✅ Papers accessible: ${papers.length} papers found`);

    const questionsCheckRes = await fetch(`${supabaseUrl}/rest/v1/questions?paper_id=eq.1&limit=10`, {
      headers: {
        'Authorization': `Bearer ${anonKey}`,
        'apikey': anonKey,
      }
    ...
    console.log(`✅ Questions accessible: ${questions.length} questions found for AIBE 20\n`);

    console.log('🎉 Database seeding complete!');
    console.log('📝 Refresh the browser at http://localhost:5173/test/20 to see the test page load.\n');
